...
{
	GLCALL(glUniformMatrix4fv(GetUniformLocation(name), 1, GL_FALSE, &matrix[0][0])); // gl_false here decides if the matrix will be transposed or not.Because we using glm s matrix, it is already column majored (what a gpu expects) . So no need for transpose.
}
